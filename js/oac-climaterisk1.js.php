...
$js .=<<< EOJS
	...
	function draw_graphs( currentTab, data ) {
		...
		if( currentTab == 3 ) {
			...
			var temp = $.extend(true, [], data2)
			temp.shift();
			data = [data,temp];	
		}
		var tmpLabels = null;
		if( tabDef[currentTab].dataIgnoreZero ) {
			tmpLabels = [];
			var tmpData = [];
			for( var i = 0; i < data.length; i++ ) {
				if( data[i] != 0 ) {
					tmpLabels.push( tabDef[currentTab].chartLabel[i] );
					tmpData.push( data[i] );
				}
			}
			data = tmpData;
		}
		color = $('input[name="ensophase"]:checked').parent().css('background-color');
		console.log( color );
		tabDef[currentTab].chart = tabDef[currentTab].chartFun( tabDef[currentTab].tabId+'-chart', data, color, tmpLabels || tabDef[currentTab].chartLabel, ymin, ymax, name);
	}
